...
dllPath        = resources_dir / "ForgeBridge.dll"
blend_filepath = resources_dir / "Halo Reach.blend"
scene_path     = blend_filepath / "Scene"
...
class ForgeObject(Structure):
    _fields_ = [
        ...
        ('forward',       float3),
        ('up',            float3),
        ('i_map_rel',     c_ushort), # spawnRelativeToMapIndex
        ('itemVariant',   c_ubyte),
        ('pad1',          c_ubyte),
        ('width',         c_float),
        ('length',        c_float),
...
def TryLinkBlendData():
    if 'Props' not in bpy.data.scenes:
        with bpy.data.libraries.load(str(blend_filepath)) as (data_from, data_to):
            for scene in data_from.scenes:
                bpy.ops.wm.append(filepath=str(scene_path / scene), directory=str(scene_path), filename=scene, link=True)
...
def register():
    ...
    if not exists(dllPath):
        ...
        return
    global forge
    forge = cdll.LoadLibrary(str(dllPath))
    try:
